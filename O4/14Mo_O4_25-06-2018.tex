...
\begin{document}
	...
	Dies führt zu maximaler Messgenauigkeit. %TODO true genug, präzise genug?
	...
	\begin{description}
		\item[Amperemeter/Multimeter:] Der Messwert des Betriebsstroms der Spulen wird von einem Multimeter abgelesen. 
			...
			Es ergibt sich also eine Unsicherheit von \SI{3}{mA} (rechteckige WDF). %TODO 0.01/2sqrt(3)=0.0087!=0.003
			Dabei wird angenommen, dass der eigentliche Messfehler des Gerätes dem Displayfehler gegenüber verschwindet. %TODO confirmen, aber sollte true sein und fehlte halt
		\item[Hallsonde:]  Die Stärke des Magnetfelds wird mit einer Hall-Sonde gemessen. Die Messwerte schwanken in der fünften Nachkommastelle, sodass eine Unsicherheit von \SI{30}{\mu T} angenommen wird. %TODO 0.0001/2sqrt(6) != 0.03
	...
	Die Stärke des Magnetfelds an der Position der Probe wird mit der Hallsonde in einem Winkel $\theta$ gemessen. %doppelt, obwohl du diesmal die Methoden schon lesen konntest. Einzig relevant ist, dass das theta heißt, deswegen lass ich'S drin =\SI{45}{\degree}
	Für ein $\theta=\SI{90}{\degree}$ wurde $\vec B_\text{Mess} = \SI{0,1}{mT}$ gemessen. %TODO hab das bissl umstrukturiert, weil die Aussage "es wird bei theta=45 gemessen. Bei theta=90 ergab sich blub" komisch ist
	In \cref{fig_draw} sind die Winkelverhältnisse dargestellt. 
	$\vec B$ ist das Magnetfeld senkrecht zur Probenoberfläche. 
	$\vec B_\text{Hall}$ zeigt die Messrichtung der Hallsonde an.
	...
	\begin{figure}[H]  %TODO eigentlich "in willkürlichen Einheiten" nicht "in willkürlicher Einheit"
		\includegraphics[width=0.7\textwidth]{fig_magn1} %TODO Größe anpassen
		\centering
	...
	\begin{figure}[H] %TODO eigentlich "in willkürlichen Einheiten" nicht "in willkürlicher Einheit"
		\includegraphics[width=0.7\textwidth]{fig_magn2}
		\centering
		\caption{Die Unsicherheiten sind kleiner als die Symbolgröße.} %TODO not sure, ob das erlaubt ist, hier fehlt mindestens ein hier wurde gesteigert oder so
		\label{fig_magn2}
		\centering
	\end{figure}
